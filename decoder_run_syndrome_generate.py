...
for num_cycles in cycs: 
    for error_rate in err:
        ...
        # find and locate the error model in circuitsZ
        def single_error_circuitZ(gate_ind_cnt, gate_type):
            single_error_cir = cycle_repeated.copy()
            single_error_cir.insert(gate_ind_cnt, gate_type)
            error_gate_cnt = []
            error_gate_ind = 0
            for error_gate in circuitsZ:
                if single_error_cir == error_gate:  
                    error_gate_cnt.append(error_gate_ind)
                error_gate_ind = error_gate_ind + 1
                
            # print("error_gate_cnt:", error_gate_cnt)
            assert(len(error_gate_cnt) == 1)
            
            values_HZdict = list(HZdict.values())
            mechanism_cnt = 0
            for err_mechanism in values_HZdict:      
                
                mechanism = np.array(err_mechanism)
                # print("mechanism:", err_mechanism)
                isin = np.isin(error_gate_cnt, mechanism)
                # # print(isin)
                if isin.any() == True:
                    return mechanism_cnt
                mechanism_cnt = mechanism_cnt + 1
                
            

        # find and locate the error model in circuitsX
        def single_error_circuitX(gate_ind_cnt, gate_type):
            single_error_cir = cycle_repeated.copy()
            single_error_cir.insert(gate_ind_cnt, gate_type)
            error_gate_cnt = []
            error_gate_ind = 0
            for error_gate in circuitsX:
                if single_error_cir == error_gate:  
                    error_gate_cnt.append(error_gate_ind)
                error_gate_ind = error_gate_ind + 1
            ...
            mechanism_cnt = 0
            for err_mechanism in values_HXdict:      
                mechanism = np.array(err_mechanism)
                if np.isin(error_gate_cnt, mechanism).any() == True:
                    return mechanism_cnt
                mechanism_cnt = mechanism_cnt + 1
        ...
        def generate_noisy_circuit(p):
            ...
            err_cnt=0
            gate_ind_cnt = 0
            error_mechanismX_cnt = []
            error_mechanismZ_cnt = []
            
            
            for gate in cycle_repeated:
                assert(gate[0] in ['CNOT','IDLE','PrepX','PrepZ','MeasX','MeasZ'])
                gate_ind_cnt = gate_ind_cnt + 1
                
                if gate[0]=='MeasX':
                    if np.random.uniform()<=error_rate_meas:
                        circ.append(('Z',gate[1]))
                        err_cnt+=1
                        error_mechanismZ_cnt.append(single_error_circuitZ(gate_ind_cnt - 1, ('Z',gate[1])))
                ...
                if gate[0]=='IDLE':
                    if np.random.uniform()<=error_rate_idle:
                        ...
                        if ptype==0:
                            ...
                            error_mechanismX_cnt.append(single_error_circuitX(gate_ind_cnt, ('X',gate[1])))
                        if ptype==1:
                            circ.append(('Y',gate[1]))
                            error_mechanismZ_cnt.append(single_error_circuitZ(gate_ind_cnt, ('Z',gate[1])))
                            error_mechanismX_cnt.append(single_error_circuitX(gate_ind_cnt, ('X',gate[1])))
                        if ptype==2:
                            circ.append(('Z',gate[1]))
                            error_mechanismZ_cnt.append(single_error_circuitZ(gate_ind_cnt, ('Z',gate[1])))
                ...
                if gate[0]=='CNOT':
                    ...
                    if np.random.uniform()<=error_rate_cnot:
                        ...
                        if error_type==3:
                            ...
                            error_mechanismX_cnt.append(single_error_circuitX(gate_ind_cnt, ('X',gate[2])))
                            err_cnt+=1
                            continue
                        if error_type==4:
                            circ.append(('Y',gate[2]))
                            error_mechanismX_cnt.append(single_error_circuitX(gate_ind_cnt, ('X',gate[2])))
                            error_mechanismZ_cnt.append(single_error_circuitZ(gate_ind_cnt, ('Z',gate[2])))
                            err_cnt+=1
                            continue
                        if error_type==5:
                        ...
                        if error_type==6:
                            ...
                            error_mechanismX_cnt.append(single_error_circuitX(gate_ind_cnt, ('XX',gate[1],gate[2])))
                            err_cnt+=1
                            continue
                        if error_type==7:
                            circ.append(('YY',gate[1],gate[2]))
                            error_mechanismX_cnt.append(single_error_circuitX(gate_ind_cnt, ('XX',gate[1],gate[2])))
                            error_mechanismZ_cnt.append(single_error_circuitZ(gate_ind_cnt, ('ZZ',gate[1],gate[2])))
                            err_cnt+=1
                            continue
                        if error_type==8:
                ...
                if gate[0]=='MeasZ':
                    if np.random.uniform()<=error_rate_meas:
                        circ.append(('X',gate[1]))
                        error_mechanismX_cnt.append(single_error_circuitX(gate_ind_cnt - 1, ('X',gate[1])))
                        err_cnt+=1
                    circ.append(gate)
                    continue
